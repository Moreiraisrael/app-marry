...
def start_server(port=3000):
    if PID_FILE.exists():
        try:
            ...
            if is_running(pid):
                print(f"Warning: Preview already running (PID: {pid})")
                return
        except:
    ...
    # Add port argument to command if it's npm
    final_cmd = cmd.copy()
    if final_cmd[0] == "npm":
        final_cmd.extend(["--", "--port", str(port)])
    ...
    print(f"Success: Preview started! (PID: {process.pid})")
...
def status_server():
    ...
    print("\n=== Preview Status ===")
    if running:
        print(f"Success: Status: Running")
        print(f"PID: {pid}")
